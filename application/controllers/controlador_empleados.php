...
class Controlador_empleados extends CI_Controller {
    ...
    private function get_datos_sesion($codigo){
        ...
        $data["Usuario_Empleado"]=$empleado->Usuario;
    ...
    private function get_empleado_post($codigo){
        $data=$this->get_datos_sesion($codigo);
        $data['Codigo']=$this->modelo_empleados->get_codigo();//$this->input->post('codigo');
    ...
    private function set_validacion_mensajes(){
        $this->form_validation->set_message('required','El campo %s es obligatorio');
    }
    public function agregar_empleado($codigo){
        $data=$this->get_empleado_post($codigo);
        $data['Titulo']="Registro de  Empleados";
        $data['Controlador']=base_url()."index.php/controlador_empleados/agregar_empleado/".$codigo;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect('controlador_empleados/index/'.$codigo);   
         }
    }
   private function get_empleado($consulta,$codigo){
       $data=$this->get_datos_sesion($codigo) ;
       if($consulta){
    ...
    public function editar_empleado($id_empleado='',$codigo, $bandera=TRUE){
        if($id_empleado!=""){
            if($bandera===TRUE){
                $data=$this->get_empleado($this->modelo_empleados->get_empleado_por_id($id_empleado),$codigo);
            }else{
                $data=$this->get_empleado_post($codigo);
                $data['Codigo']=$this->input->post('codigo');
            }
            $data['Titulo']="Edición de  Empleado";
            $data['Controlador']=base_url()."index.php/controlador_empleados/editar_empleado/".$id_empleado."/".$codigo."/FALSE";
    ...
    public function eliminar_empleado($id='',$codigo){
